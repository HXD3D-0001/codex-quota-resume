...
- 360×38 逻辑像素的磨砂玻璃悬浮条，高 DPI 绘制，文字保持完全不透明。鼠标离开顶部识别区后完全隐藏，再移入时展开；在系统托盘可选择常驻显示。隐藏时不保留磨砂小窗，避免残留方框。
- 悬浮条完全点击穿透、不抢焦点；点击被遮住的内容直接落到下面的应用。刷新、暂停、启用、退出均放在系统托盘右键菜单。
- 自动刷新不依赖点击或对话：后台持续查询；倒计时每秒更新。重置后服务端暂未更新时显示“更新中”并每 5 秒重查，不伪造 100% 余量。
- 状态点：绿色表示监测正常、当前扫描未发现待恢复任务；蓝色表示有待恢复任务；灰色表示自动续跑暂停；黄色表示连接/数据、扫描范围或发送回执需要检查。绿色不是全部历史任务从未发生限额中断的保证。完整说明可查看托盘提示。
